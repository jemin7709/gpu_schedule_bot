...
- 외부 서비스·라이브러리·인프라(예: 별도 DB, 메시지 큐)를 추가할 때는 이후 유지보수 복잡도와 장애 가능성을 고려해, 기본적으로는 Slack, Google 서비스, 로컬 실행 환경 3가지를 벗어나지 않는 구성을 우선 검토한다 (MUST).
- 언제든 기능을 제거하거나 단순화할 수 있도록, 설정·코드·문서를 최소한으로 유지하고 중복을 피하며, 불필요해진 요소는 다음 작업 시 우선적으로 정리한다 (SHOULD).
